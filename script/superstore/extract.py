def extract_table(table, **kwargs):
   ...
   with engine.connect() as conn:
       ...
       query = f"SELECT *, CURRENT_TIMESTAMP AS md_extracted_at FROM SuperStore.{table}"
   ...
   os.makedirs(f"data/superstore/{table}", exist_ok=True)
   df.to_parquet(f"data/superstore/{table}/{data_interval_start}_{data_interval_end}_{ingestion_mode}.parquet", index=False)
