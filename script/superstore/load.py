def load_table(table, **kwargs):
   ...
   df = pd.read_parquet(f"data/superstore/{table}/{data_interval_start}_{data_interval_end}_{ingestion_mode}.parquet")

   engine = PostgresHook("postgres_superstore").get_sqlalchemy_engine()
   with engine.connect() as conn:
       ...
       df.to_sql(table, conn, schema="superstore", index=False, if_exists=if_exists)
